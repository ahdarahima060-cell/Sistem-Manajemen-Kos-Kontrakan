...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col">
            <h2 class="fw-bold">Tambah Kontrak Penyewa</h2>
            <p class="text-secondary">Isi detail kontrak baru untuk penyewa.</p>
        </div>
    </div>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-header bg-pink text-white border-0">
            <div class="d-flex align-items-center justify-content-between">
                <div>
                    <h5 class="mb-0">Form Tambah Kontrak</h5>
                    <small>Isi data kontrak penyewa dengan lengkap.</small>
                </div>
            </div>
        </div>
        <div class="card-body">
            <form action="{{ route('kontrak.store') }}" method="POST">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="room_id" required>
                            ...
                            @foreach($rooms as $room)
                                <option value="{{ $room->id }}" {{ old('room_id') == $room->id ? 'selected' : '' }}>{{ $room->room_code }} - {{ $room->type }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control" name="monthly_rent" value="{{ old('monthly_rent') }}" step="0.01" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Deposit</label>
                        <input type="number" class="form-control" name="deposit_paid" value="{{ old('deposit_paid') }}" step="0.01" required>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="payment_status" required>
                            ...
                            <option value="paid_ahead" {{ old('payment_status') == 'paid_ahead' ? 'selected' : '' }}>paid_ahead</option>
                        </select>
                    </div>
                ...
                <div class="mt-4 d-flex gap-2">
                    <button type="submit" class="btn btn-pink px-4">Simpan Kontrak</button>
